    ...
    <section class="profile__posts tabs__content tabs__content--active">
        ...
        <article class="profile__post post <?= $post['icon_name'] ?>">
            ...
            if ($post['repost']): ?>
                <header class="post__header">
                    <div class="post__author">
                        <a class="post__author-link" href="profile.php?user=<?= $post['original']['id'] ?>"
                           title="Автор">
                            <div class="post__avatar-wrapper post__avatar-wrapper--repost">
                                <img class="post__author-avatar" src="<?= $post['original']['avatar'] ?>"
                                     alt="Аватар пользователя">
                            </div>
                            <div class="post__info">
                                <b class="post__author-name">Репост: <?= $post['original']['login'] ?></b>
                                <time class="post__time" datetime="<?= $post['create_date'] ?>"> <?= smallDate(
                                        $post['create_date'],
                                        'назад'
                                    ) ?></time>
                            </div>
            ...
            <?php
                ...
                <header class="post__header">
                    <h2><a href="post.php?post-id=<?= $post['post_num'] ?>"><?= $post['login'] ?></a></h2>
                </header>
            <?php
            endif; ?>
                    ...
                    <div class="post-link__wrapper">
                        <a class="post-link__external" href="<?= $post['media'] ?>" title="Перейти по ссылке">
                            <div class="post-link__icon-wrapper">
                                <img src="https://www.google.com/s2/favicons?domain=<?= $post['media'] ?>" alt="Иконка">
                            </div>
                ...
                <div class="post__indicators">
                    <div class="post__buttons">
                        ...
                        <a class="post__indicator post__indicator--repost button"
                           href="repost.php?postId=<?= $post['post_num'] ?>" title="Репост">
                            <svg class="post__indicator-icon" width="19" height="17">
                                <use xlink:href="#icon-repost"></use>
                            </svg>
                            <span><?= $post['repost_count'] ?></span>
                            <span class="visually-hidden"><?= $post['repost_count'] ?></span>
                        </a>
                    </div>
                    <time class="post__time" datetime="<?= smallDate($post['create_date'], 'назад') ?>"><?= smallDate(
                            $post['create_date'],
                            'назад'
                        ) ?></time>
                </div>
            ...
            if (!$isComment): ?>
                <div class="comments">
                    <a class="comments__button button" href="profile.php?user=<?= $userId ?>&comment=enabled">Показать
                        комментарии</a>
                </div>
            <?php
            else: ?>
                ...
                if (($post['count_comments'] - 2) > 0): ?>
                    <div class="comments">
                        <div class="comments__list-wrapper">
                            ...
                            <?php
                            if (($post['count_comments'] - 2) > 0 && !$isCommentShowAll): ?>
                                <a class="comments__more-link"
                                   href="profile.php?user=<?= $userId ?>&comment=enabled&view=all">
                                    <span>Показать все комментарии</span>
                                    <sup class="comments__amount"><?= $post['count_comments'] - 2 ?></sup>
                                </a>
                            <?php
                            endif ?>
                        </div>
                    ...
                    <form class="comments__form form" action="add-comment.php" method="post">
                        <div class="comments__my-avatar">
                            <img class="comments__picture" src="<?= $post['avatar'] ?>" alt="Аватар пользователя">
                        </div>
                        ...
                        <button class="comments__submit button button--green" name="postId"
                                value="<?= $post['post_num'] ?>" type="submit">Отправить
                        </button>
                    </form>
                <?php
                endif; ?>
