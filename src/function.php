...
//добавляем многоточия в массив
        ...
        $result[] = $ellipsis;
//добавляем ссылку
        $link = '<a class="post-text__more-link"  href="post.php?post-id='.$url
            .'">Читать далее</a>';
...
//вычисляем разницу между серверным временем и датой поста
    ...
    if ($difference->m > 0) {
        $resultForPost = getNounPluralForm(
            $difference->m,
            'месяц',
            'месяца',
            'месяцев'
...
function layoutContentDefine()
{
    ...
    } elseif ($content === '/messages.php') {
        return 'page__main--messages';
    }
}
...
function getClassNameAddForm(?int $type, array $types)
{
    foreach ($types as $key => $contentTypes) {
        if ($contentTypes['id'] === $type) {
            return 'adding-post__'.$contentTypes['type_name']
